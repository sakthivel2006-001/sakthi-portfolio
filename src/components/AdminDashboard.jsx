...
const AdminDashboard = ({ isOpen, onClose }) => {
  ...
  const fetchMessages = async () => {
    try {
      ...
      console.warn("Using localStorage for messages fallback");
  ...
  const fetchComments = async () => {
    try {
      ...
      console.warn("Using localStorage for comments fallback");
      const localComments = JSON.parse(localStorage.getItem('portfolioComments') || '[]');
  ...
  const deleteMessage = async (id) => {
    ...
    try {
      ...
      console.warn("Using localStorage for deleteMessage fallback");
      const localMessages = JSON.parse(localStorage.getItem('portfolioMessages') || '[]');
      const filtered = localMessages.filter(msg => msg.id !== id);
      localStorage.setItem('portfolioMessages', JSON.stringify(filtered));
      alert('Message deleted successfully (locally)!');
      fetchMessages();
    }
  };
  ...
  const deleteComment = async (id) => {
    ...
    try {
      ...
      console.warn("Using localStorage for deleteComment fallback");
      const localComments = JSON.parse(localStorage.getItem('portfolioComments') || '[]');
      const filtered = localComments.filter(comment => comment.id !== id);
      localStorage.setItem('portfolioComments', JSON.stringify(filtered));
      alert('Comment deleted successfully (locally)!');
      fetchComments();
    }
  };
                        ...
                        <div className="flex items-start justify-between mb-3">
                          ...
                          <div className="flex gap-2">
                            ...
                            <button
                              ...
                              className="p-2 bg-red-600 hover:bg-red-500 rounded-lg transition-colors duration-300 flex items-center justify-center"
                              title="Delete"
                            >
                              <FaTrash className="text-white" />
                            </button>
                        ...
                        <AnimatePresence>
                          {replyingToId === msg.id && (
                            <motion.div
                              ...
                              className="mt-4 pt-4 border-t border-slate-700/50 flex flex-col gap-3"
                            >
                              <textarea
                                ...
                                rows="3"
                                className="w-full bg-slate-900/50 border border-blue-500/30 rounded-xl p-4 text-slate-200 focus:outline-none focus:border-blue-400 focus:ring-1 focus:ring-blue-400 transition-all resize-none"
                              ></textarea>
                              <div className="flex justify-end gap-2">
                                <button
                                  onClick={() => setReplyingToId(null)}
                                  className="px-4 py-2 rounded-lg text-slate-400 hover:text-slate-200 hover:bg-slate-700 transition-all"
                                >
                                  Cancel
                                </button>
                                <button
                                  ...
                                  className="px-6 py-2 bg-blue-600 hover:bg-blue-500 disabled:opacity-50 text-white rounded-lg transition-all flex items-center gap-2"
                                >
                                  {isSendingReply ? (
